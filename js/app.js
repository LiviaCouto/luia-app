...
function renderRevisoes(){
  ...
  // Filtra revisões pela prova ativa — declarado aqui para todo o escopo da função
  const pid=typeof getProvaAtivaId==='function'?getProvaAtivaId():null;
  const _revFiltradas=reviews.filter(r=>pid?(r.prova_id||'ifpe')===pid:true);
...
async function init(){
  ...
  initStars();
  // Inicializa config de provas e atualiza labels
  if(typeof initConfig==='function') initConfig();
  // Atualiza label do menu com a prova ativa
  const _provaInicio=typeof getProvaAtiva==='function'?getProvaAtiva():null;
  if(_provaInicio){
    const _pl=document.getElementById('plan-label');
    const _mn=document.getElementById('menu-prova-nome');
    if(_pl) _pl.textContent=_provaInicio.nome;
    if(_mn) _mn.textContent=_provaInicio.nome;
  }
  renderCronograma();
...
function _renderFiltroProva(containerId, callbackFn) {
  ...
  const opcoes = [{ id:'ativa', nome:'Prova ativa', icone:'✦' }];
